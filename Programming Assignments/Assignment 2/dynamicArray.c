...
void addDynArr(DynArr *v, TYPE val)
{
	...
	if(v->size >= v->capacity)
	{
		...
		struct DynArr newArray;
		initDynArr(&newArray, v->capacity * 2);
		
		//copy data from old array into new
		for(int i = 0; i < v->capacity; i++)
		{
			newArray.data[i] = v->data[i];
		}
...
int containsDynArr(DynArr *v, TYPE val)
{
	...
	for(i = 0; i < v->size; i++)
	{
		if(v->data[i] == val) //if v is found, return 1 (true)
		{
			return 1;
		}
	}
...
void removeDynArr(DynArr *v, TYPE val)
{
	...
	if(v->size != 0) //if bag is empty, do nothing
	{
		for(i = 0; i < v->size; i++)
		{
			...
			if(v->data[i] == val) 
			{
				//unless it is the last element in the array, then just decrement size and return
				if(i == v->size - 1)
				{
					v->size--;
					return;
				...
				//otherwise, start with index to the right of i, copy its value to the left
				//this results in a duplicate value at the tail of the array, but it
				//will be overwritten by the next add operation because count was decremented
